...
class FamFinanceSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $members = [
            ['Siti Pratiwi', 'siti.pratiwi@email.com', 'siti.pratiwi', 'Ibu', '0812-1111-2222', true],
            ['Raka Pratama', 'raka.pratama@email.com', 'raka.pratama', 'Anak', '0812-3333-4444', true],
            ['Ayu Pratama', 'ayu.pratama@email.com', 'ayu.pratama', 'Admin Keluarga', '0812-5555-6666', false],
        ];

        foreach ($members as [$name, $email, $username, $roleName, $phone, $isActive]) {
            User::updateOrCreate([
                'email' => $email,
            ], [
                'family_id' => $family->id,
        ...
        $samples = [
            ['Gaji Bulanan', 'income', 'Gaji', 15000000, 'BCA', 'bank', 'success', $period->copy()->day(min(5, $period->daysInMonth))->toDateString()],
            ['Listrik PLN', 'expense', 'Listrik', 450000, 'BCA', 'bank', 'success', $period->copy()->day(min(6, $period->daysInMonth))->toDateString()],
            ['Belanja di Superindo', 'expense', 'Belanja Rumah Tangga', 320000, 'Cash', 'cash', 'success', $period->copy()->day(min(8, $period->daysInMonth))->toDateString()],
            ['Internet Indihome', 'expense', 'Internet', 250000, 'BCA', 'bank', 'success', $period->copy()->day(min(10, $period->daysInMonth))->toDateString()],
            ['BPJS Kesehatan', 'expense', 'BPJS', 150000, 'BCA', 'bank', 'success', $period->copy()->day(min(12, $period->daysInMonth))->toDateString()],
            ['Imunisasi Anak', 'expense', 'Imunisasi', 200000, 'Cash', 'cash', 'success', $period->copy()->day(min(14, $period->daysInMonth))->toDateString()],
            ['Freelance Design', 'income', 'Freelance', 1200000, 'OVO', 'e-wallet', 'success', $period->copy()->day(min(18, $period->daysInMonth))->toDateString()],
        ];

        foreach ($samples as $index => [$title, $type, $categoryName, $amount, $walletName, $method, $status, $date]) {
            $transaction = Transaction::updateOrCreate([
                'transaction_code' => 'TRX-'.$period->format('Ym').'-'.str_pad((string) ($index + 1), 5, '0', STR_PAD_LEFT),
            ], [
                'family_id' => $family->id,
                'user_id' => $user->id,
                ...
                'amount' => $amount,
                'title' => $title,
                'description' => $title.' '.$period->translatedFormat('F Y'),
